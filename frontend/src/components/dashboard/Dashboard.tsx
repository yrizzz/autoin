...
export default function Dashboard() {
  ...
  const maxAnalyticVal = Math.max(...finalBcAnalytic.map(d => d.total), 10);
            ...
            <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-6 mb-8 shadow-sm">
              ...
              <div className="h-48 relative">
                ...
                <svg className="w-full h-full" viewBox="0 0 720 160" preserveAspectRatio="none">
                  ...
                  {/* Total Area & Line */}
                  <path 
                    d={`M 30 140 ${finalBcAnalytic.map((day, idx) => {
                      const x = 30 + (idx / (finalBcAnalytic.length - 1)) * 660;
                  ...
                  {/* Interactive Dots */}
                  {finalBcAnalytic.map((day, idx) => {
                    const x = 30 + (idx / (finalBcAnalytic.length - 1)) * 660;
                    const totalY = 140 - (day.total / maxAnalyticVal) * 110;
                    const sentY = 140 - (day.sent / maxAnalyticVal) * 110;
                    return (
                      <g key={idx} className="group/dot cursor-pointer">
                        <circle cx={x} cy={totalY} r="4" fill="#3b82f6" stroke="white" strokeWidth="1.5" />
                        <circle cx={x} cy={sentY} r="4" className="fill-blue-600 dark:fill-blue-400" stroke="white" strokeWidth="1.5" />
                      </g>
                    );
                  })}
                </svg>
                ...
                <div className="flex justify-between px-7 mt-2">
                  {finalBcAnalytic.map((day, idx) => (
                    <span key={idx} className="text-[9px] text-zinc-400 dark:text-zinc-500 font-bold uppercase font-mono">{day.date}</span>
                  ))}
                </div>
              </div>
            </div>
                ...
                {[
                  ...
                  return (
                    <a
                      key={idx}
                      href={item.href}
                      className="group relative flex items-center gap-4 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5 transition-all duration-300 hover:bg-zinc-50 dark:hover:bg-zinc-850 hover:border-zinc-300 dark:hover:border-zinc-700 hover:-translate-y-0.5 shadow-sm"
                    >
                      ...
                      return (
                        <div key={idx} className="flex-1 flex flex-col items-center h-full justify-end group cursor-pointer">
                          <div className="w-full flex items-end justify-center gap-1 h-[80%] relative">
                            {/* Hover tooltip */}
                            <div className="absolute bottom-full mb-2 bg-zinc-900 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 text-white rounded-lg p-2 text-[9px] opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20 shadow-xl min-w-[80px] text-center">
