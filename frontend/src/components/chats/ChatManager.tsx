...
import {
  ...
  FileText, Film, Music, MoreVertical, User
} from 'lucide-react';

interface Chat { id: string; name: string; lastMessage: string; time: string; unread: number; ts?: number; }
interface Msg { id: string; sender: 'me'|'them'; text: string; time: string; status: 'sending'|'sent'|'delivered'; mediaUrl?: string; mediaType?: string; }
...
const isGrp = (id: string) => id.endsWith('@g.us');
const phone = (id: string) => '+' + id.split('@')[0];
...
export default function ChatManager() {
  ...
  useEffect(() => {
    api.get<Channel[]>('/api/channels').then(chs => {
      ...
      setChannels(wa); if (wa.length) setCh(wa[0]); else setChatsL(false);
    }).catch(() => setChatsL(false));
  }, []);

  // Load chats + subscribe SSE chat-list updates
  useEffect(() => {
    if (!ch) return;
    setChatsL(true); setChats([]); setActive(null); setMsgs([]);
    api.get<{ chats: Chat[] }>(`/api/whatsapp/${ch.id}/chats`)
      .then(r => setChats(r.chats || [])).catch(() => setChats([]))
      .finally(() => setChatsL(false));

    // SSE: chat list updates
    if (sseCht.current) sseCht.current.close();
    ...
    esChats.onerror = () => {
      ...
      if (sessId) {
        console.warn('Laravel SSE chats stream failed, falling back directly to Node.js WA Service');
        esChats.close();
        const fallbackUrl = `http://localhost:3001/sessions/${sessId}/events/chats?secret=autoin-wa-secret`;
        const fallbackEs = new EventSource(fallbackUrl);
        setupChatsListeners(fallbackEs);
        sseCht.current = fallbackEs;
      }
    };

    sseCht.current = esChats;
    return () => { if (sseCht.current) sseCht.current.close(); };
  }, [ch?.id]);
  ...
  // Load msgs + SSE per-chat
  const loadMsgs = useCallback(async (channel: Channel, chat: Chat) => {
    try {
      const r = await api.get<{ messages: Msg[] }>(`/api/whatsapp/${channel.id}/messages/${encodeURIComponent(chat.id)}`);
      setMsgs(r.messages || []);
    } catch { setMsgs([]); }
  }, []);
  ...
  useEffect(() => {
    ...
    // SSE: incoming messages for this chat
    if (sseMsg.current) sseMsg.current.close();
    const token = localStorage.getItem('autoin_token');
    ...
    esMsgs.onerror = () => {
      const sessId = ch.credentials?.session_id;
      if (sessId) {
        console.warn('Laravel SSE message stream failed, falling back directly to Node.js WA Service');
        esMsgs.close();
        const fallbackUrl = `http://localhost:3001/sessions/${sessId}/events/messages/${encodeURIComponent(active.id)}?secret=autoin-wa-secret`;
        const fallbackEs = new EventSource(fallbackUrl);
        setupMsgListeners(fallbackEs);
        sseMsg.current = fallbackEs;
      }
    };

    sseMsg.current = esMsgs;
    return () => { if (sseMsg.current) sseMsg.current.close(); };
  }, [active?.id, ch?.id]);
  ...
  // Filtered + categorised chat list
  const visible = chats.filter(c => {
    if (tab === 'personal' && isGrp(c.id)) return false;
    if (tab === 'group' && !isGrp(c.id)) return false;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1 flex items-center gap-1.5">
            ...
            Real-time via SSE · teks, gambar, video, audio, dokumen
          </p>
        </div>
        {channels.length > 1 && (
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-zinc-100 dark:divide-zinc-800/50">
              {chatsLoading ? (
                ...
                <div className="flex flex-col items-center justify-center h-full gap-2 px-4 text-center">
                  ...
                  <p className="text-xs text-zinc-500 dark:text-zinc-400">{search ? 'Tidak ada hasil.' : tab === 'group' ? 'Tidak ada grup.' : tab === 'personal' ? 'Tidak ada chat personal.' : 'Belum ada obrolan.'}</p>
                </div>
              ) : (
                visible.map(c => {
                  const isA = active?.id === c.id;
                  const bg = hue(c.id);
                  const grp = isGrp(c.id);
                  return (
                    <div key={c.id} onClick={() => setActive(c)}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-bold text-zinc-900 dark:text-zinc-100 truncate">{c.name || phone(c.id)}</span>
            ...
            {active ? (
              <>
                ...
                <div className="h-14 px-5 flex items-center gap-3 bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 shrink-0">
                  <div className={`w-9 h-9 rounded-full ${hue(active.id)} flex items-center justify-center text-white font-bold text-sm shrink-0`}>
                    {isGrp(active.id) ? <Users className="w-4 h-4" /> : (active.name || active.id)[0].toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-bold text-zinc-900 dark:text-zinc-100 truncate flex items-center gap-2">
                      {active.name || phone(active.id)}
                      {isGrp(active.id)
                    ...
                    <div className="text-[10px] text-zinc-400 font-mono">{phone(active.id)}</div>
                  </div>
                  ...
                  <button onClick={() => loadMsgs(ch!, active)} className="p-2 text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition-all"><RefreshCw className="w-4 h-4" /></button>
                </div>
                  ...
                  <div className="px-4 py-2 bg-zinc-50 dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-800 flex items-center gap-3">
                    <div className="flex-1 flex items-center gap-2 min-w-0">
                      {att.preview ? <img src={att.preview} className="w-10 h-10 object-cover rounded-lg shrink-0" alt="" />
                        : <div className="w-10 h-10 bg-zinc-200 dark:bg-zinc-700 rounded-lg flex items-center justify-center shrink-0">{att.mediaType==='video'?<Film className="w-5 h-5 text-purple-500"/>:att.mediaType==='audio'?<Music className="w-5 h-5 text-green-500"/>:<FileText className="w-5 h-5 text-orange-500"/>}</div>}
                      <div className="min-w-0"><div className="text-xs font-semibold text-zinc-700 dark:text-zinc-300 truncate">{att.name}</div><div className="text-[10px] text-zinc-400 capitalize">{att.mediaType}</div></div>
                    </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center p-8">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-2 max-w-xs">Klik obrolan dari daftar di sebelah kiri.</p>
                </div>
                <div className="text-[10px] text-zinc-400 flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                  Pesan baru muncul otomatis via SSE
                </div>
              </div>
            )}
          </div>
