...
export default function LandingFeatures() {
  return (
    <section id="features" className="relative py-28 px-6 bg-[#030712]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {features.map((f, i) => {
            ...
            return (
              <div
                key={i}
                className="group relative rounded-2xl border border-white/5 bg-[#0a0f1d]/40 p-8 backdrop-blur-sm transition-all duration-300 hover:border-white/10 hover:bg-[#0c142c]/50 hover:-translate-y-1 hover:shadow-2xl"
              >
                ...
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center bg-white/5 border border-white/8 mb-6 group-hover:scale-110 transition-transform duration-300 ${f.iconColor}`}>
                  <Icon className="w-5 h-5" />
                </div>
                
                <h3 className="font-semibold text-lg text-white mb-3 tracking-tight group-hover:text-gradient-brand transition-colors duration-300">
                  {f.title}
                </h3>
